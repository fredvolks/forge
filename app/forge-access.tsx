...
export type ForgeSession = { companyId: string; companyName: string; userName: string; email: string; role: 'Boss' | 'Adjointe' | 'Chef' | 'Employé'; logo?: string };
...
export function ForgeAccess({ onEnter }:{ onEnter:(session:ForgeSession)=>void }) {
  ...
  function createCompany(e:React.FormEvent<HTMLFormElement>) {
    ...
    const session:ForgeSession={companyId:id,companyName:String(form.get('company')),userName:String(form.get('owner')),email:String(form.get('loginEmail')),role:'Boss',logo};
