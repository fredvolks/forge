...
type CenterOrder={id:string;companyId:string;date:string;job:string;requester:string;items:Array<{name:string;qty:number;unit:string;source:'Inventaire MIR'|'Fournisseur';supplier?:string;detail?:string}>;status:string;history:string[]};
type Supplier={name:string;rep:string;email:string;phone:string;address:string;notes?:string;active?:boolean};
const baseOrders:CenterOrder[]=[
...
const generatedOrders:CenterOrder[]=Array.from({length:180},(_,index)=>{const date=new Date(2026,6,1+(index%64));const jobs=['JOB-214 · Breton','JOB-315 · Leduc','JOB-418 · Bélanger','JOB-193 · Laurentien'];const people=['Fred G.','Alex P.','Marco T.','Samir B.','Patrick D.','Nicolas R.'];const states=['Reçue','À traiter','En préparation','Commandée fournisseur','En attente','Prête','Embarquée','Complétée'];const products=[['J soffite','morceaux','Inventaire MIR'],['Boîte de soffite','boîtes','Fournisseur'],['Moulure coupe-feu','morceaux','Fournisseur'],['Lame Olfa 1″','paquets','Inventaire MIR'],['Clou 3¼','boîtes','Inventaire MIR'],['Fascia','morceaux','Fournisseur']] as const;const first=products[index%products.length];const second=products[(index+2)%products.length];return{id:`BC-2026-${String(600+index).padStart(4,'0')}`,companyId:'mir-demo',date:date.toISOString().slice(0,10),job:jobs[index%jobs.length],requester:people[index%people.length],status:states[index%states.length],history:[`Créée par ${people[index%people.length]}`,states[index%states.length]],items:[{name:first[0],qty:2+(index%18),unit:first[1],source:first[2],supplier:first[2]==='Fournisseur'?'Acier Breton':undefined},{name:second[0],qty:1+(index%7),unit:second[1],source:second[2],supplier:second[2]==='Fournisseur'?'Acier Breton':undefined}]}});
const seedOrders:CenterOrder[]=[...baseOrders,...generatedOrders];
const catalogSeed=[
...
const supplierSeed:Supplier[]=[{name:'Acier Breton',rep:'Martin Côté',email:'commandes@acierbreton.ca',phone:'418 555-0184',address:'Lévis',notes:'Livraison chantier en 2 jours',active:true},{name:'Quincaillerie Pro',rep:'Julie Roy',email:'commandes@quincailleriepro.ca',phone:'418 555-0122',address:'Québec',active:true}];
const statuses=['Reçue','À traiter','En préparation','Commandée fournisseur','En attente','Prête','Embarquée','Complétée'];

export function CommandCenter({role,companyId,initialTab='Commandes',showNav=true,initialOrderId,initialStatus='Tous',lockedJobNumber}:{role:Role;companyId:string;initialTab?:string;showNav?:boolean;initialOrderId?:string;initialStatus?:string;lockedJobNumber?:string}){
 const [tab,setTab]=useState(initialTab); const [orders,setOrders]=useState<CenterOrder[]>([]); const [catalog,setCatalog]=useState(catalogSeed); const [inventory,setInventory]=useState(inventorySeed); const [suppliers,setSuppliers]=useState(supplierSeed); const [query,setQuery]=useState(''); const [status,setStatus]=useState('Tous'); const [jobFilter,setJobFilter]=useState('Toutes'); const [employeeFilter,setEmployeeFilter]=useState('Tous'); const [supplierFilter,setSupplierFilter]=useState('Tous'); const [sourceFilter,setSourceFilter]=useState('Toutes'); const [yearFilter,setYearFilter]=useState('2026'); const [monthFilter,setMonthFilter]=useState('Tous'); const [selected,setSelected]=useState<string|null>(null);
 useEffect(()=>setTab(initialTab),[initialTab]);
 useEffect(()=>{setSelected(initialOrderId||null);setStatus(initialStatus);setYearFilter('Toutes les années')},[initialOrderId,initialStatus]);
 useEffect(()=>{const load=()=>{const raw=localStorage.getItem(`forge:${companyId}:orders`);const stored=(raw?JSON.parse(raw):[]) as CenterOrder[];setOrders(stored.filter(o=>o.companyId===companyId))};load();const rawCatalog=localStorage.getItem(`forge:${companyId}:catalog`);if(rawCatalog)setCatalog(JSON.parse(rawCatalog));const rawInventory=localStorage.getItem(`forge:${companyId}:inventory`);if(rawInventory)setInventory(JSON.parse(rawInventory));const rawSuppliers=localStorage.getItem(`forge:${companyId}:suppliers`);if(rawSuppliers)setSuppliers(JSON.parse(rawSuppliers));window.addEventListener('forge-orders-updated',load);return()=>window.removeEventListener('forge-orders-updated',load)},[companyId]);
 const save=(next:CenterOrder[])=>{setOrders(next);localStorage.setItem(`forge:${companyId}:orders`,JSON.stringify(next));window.dispatchEvent(new Event('forge-orders-updated'))};
 const saveCatalog=(next:typeof catalog)=>{setCatalog(next);localStorage.setItem(`forge:${companyId}:catalog`,JSON.stringify(next));window.dispatchEvent(new Event('forge-catalog-updated'))};
 const saveInventory=(next:typeof inventory)=>{setInventory(next);localStorage.setItem(`forge:${companyId}:inventory`,JSON.stringify(next))};
 const saveSuppliers=(next:Supplier[])=>{setSuppliers(next);localStorage.setItem(`forge:${companyId}:suppliers`,JSON.stringify(next))};
 const scopedOrders=orders.filter(o=>!lockedJobNumber||o.job.split(' · ')[0]===lockedJobNumber);
 const filtered=scopedOrders.filter(o=>(!lockedJobNumber||o.job.split(' · ')[0]===lockedJobNumber)&&(status==='Tous'||(status==='À recevoir / traiter'&&['Reçue','À traiter'].includes(o.status))||o.status===status)&&(jobFilter==='Toutes'||o.job===jobFilter)&&(employeeFilter==='Tous'||o.requester===employeeFilter)&&(supplierFilter==='Tous'||o.items.some(i=>i.supplier===supplierFilter))&&(sourceFilter==='Toutes'||o.items.some(i=>i.source===sourceFilter))&&(yearFilter.startsWith('Toutes')||o.date.startsWith(yearFilter))&&(monthFilter==='Tous'||o.date.slice(5,7)===monthFilter)&&`${o.id} ${o.job} ${o.requester} ${o.items.map(i=>i.name).join(' ')}`.toLowerCase().includes(query.toLowerCase()));
 const metrics=[['NOUVELLES',scopedOrders.filter(o=>o.status==='Reçue').length],['À PRÉPARER',scopedOrders.filter(o=>['À traiter','En préparation'].includes(o.status)).length],['CHEZ FOURNISSEUR',scopedOrders.filter(o=>o.status==='Commandée fournisseur').length],['PRÊTES / EMBARQUÉES',scopedOrders.filter(o=>['Prête','Embarquée'].includes(o.status)).length],['COMPLÉTÉES',scopedOrders.filter(o=>o.status==='Complétée').length]];
 ...
 const exportCsv=()=>{const rows=[['Numéro du bon','Date','Numéro de Job','Nom de la Job','Demandé par','Article','Quantité','Unité','Source','Fournisseur','État'],...filtered.flatMap(o=>o.items.map(i=>{const [jobNumber,jobName='']=o.job.split(' · ');return[o.id,o.date,jobNumber,jobName,o.requester,i.name,String(i.qty),i.unit,i.source,i.supplier||'',o.status]}))];const csv='\uFEFF'+rows.map(r=>r.map(v=>`"${v.replaceAll('"','""')}"`).join(';')).join('\r\n');const blob=new Blob([csv],{type:'text/csv;charset=utf-8'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=`forge-commandes-${yearFilter}-${monthFilter==='Tous'?'tous-les-mois':monthFilter}.csv`;a.click();URL.revokeObjectURL(url)};
...
<option>Tous</option><option>À recevoir / traiter</option>{statuses.map(s=>
<option key={s}>{s}</option>)}</select></label>
<label className="filter-field"><span>Job</span><select aria-label="Filtrer la Job" disabled={Boolean(lockedJobNumber)} value={jobFilter} onChange={e=>setJobFilter(e.target.value)}>
...
<select aria-label="Filtrer la Job" disabled={Boolean(lockedJobNumber)} value={jobFilter} onChange={e=>setJobFilter(e.target.value)}>
