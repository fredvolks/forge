...
export function PunchPreview({role,punched,selectedJob,activeJob,startedAt,onJob,onToggle,onOpenJob,onHours,onSwitch,todayLabel,todaySegments}:{activeJob:string;onSwitch:()=>void;todayLabel:string;todaySegments:{id:string;job:string;start:string;end:string|null}[];role:'Chef'|'Employé';punched:boolean;selectedJob:string;startedAt:number|null;onJob:(v:string)=>void;onToggle:()=>void;onOpenJob:()=>void;onHours:()=>void}){
 const [now,setNow]=useState(0);const [reportOpen,setReportOpen]=useState(false);const [progress,setProgress]=useState<Record<string,number>>(()=>Object.fromEntries(progressItems));
 ...
 const switching=punched&&selectedJob!==activeJob;const clock=(value:string)=>new Date(value).toLocaleTimeString('fr-CA',{hour:'2-digit',minute:'2-digit',hourCycle:'h23'}).replace(' h ',':');
 ...
 return <div className="punch-redesign punch-v5">
  ...
  <div className="new-punch-layout"><div className="new-punch-main">
   ...
   <div className="new-punch-navigation"><a href={mapsUrl} target="_blank" rel="noreferrer" aria-label={`Ouvrir ${job.address} dans Google Maps`}><MapPin/><span>{job.address}</span><b>Google Maps</b></a><a href={wazeUrl} target="_blank" rel="noreferrer" aria-label={`Ouvrir ${job.address} dans Waze`}><Navigation/><span>Waze</span></a></div>
   ...
   <button className="new-punch-action" onClick={punch}>{switching?`PUNCH IN · ${selectedJob}`:punched?'PUNCH OUT':'PUNCH IN'}</button>
  </div><aside className="new-punch-side"><section><span>AUJOURD’HUI</span><small>Total d’heures</small><b>{todayLabel}</b><div>{todaySegments.map(s=><i key={s.id}>{s.job}</i>)}</div></section><section><span>MA JOURNÉE</span><div className="new-timeline">{todaySegments.map(s=><p key={s.id}><b>{clock(s.start)} → {s.end?clock(s.end):'En cours'}</b><small>{s.job}</small></p>)}{!todaySegments.length&&<p>Aucun segment aujourd’hui</p>}</div></section></aside></div>
