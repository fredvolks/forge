...
export function PunchPreview({role,punched,selectedJob,startedAt,onJob,onToggle,onOpenJob,onHours,onSwitch,todayLabel,todaySegments}:{onSwitch:()=>void;todayLabel:string;todaySegments:{id:string;job:string;start:string;end:string|null}[];role:'Chef'|'Employé';punched:boolean;selectedJob:string;startedAt:number|null;onJob:(v:string)=>void;onToggle:()=>void;onOpenJob:()=>void;onHours:()=>void}){
 ...
 return <div className="punch-redesign punch-v9">
  ...
  <div className="new-punch-layout"><div className="new-punch-main">
   ...
   <button className="new-punch-action" onClick={punch}>{punched?'PUNCH OUT':'PUNCH IN'}</button>{punched&&<button className="new-change-job" onClick={onSwitch}>Changer de job</button>}
  </div><aside className="new-punch-side"><section><span>AUJOURD’HUI</span><small>Total d’heures</small><b>{todayLabel}</b><div>{todaySegments.map(s=><i key={s.id}>{s.job}</i>)}</div></section><section><span>MA JOURNÉE</span><div className="new-timeline">{todaySegments.map(s=><p key={s.id}><b>{new Date(s.start).toLocaleTimeString('fr-CA',{hour:'2-digit',minute:'2-digit'})} → {s.end?new Date(s.end).toLocaleTimeString('fr-CA',{hour:'2-digit',minute:'2-digit'}):'En cours'}</b><small>{s.job}</small></p>)}{!todaySegments.length&&<p>Aucun segment aujourd’hui</p>}</div></section></aside></div>
